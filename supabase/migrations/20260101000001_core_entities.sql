...
create policy "owner access" on properties for all using (auth.uid() = owner_id) with check (auth.uid() = owner_id);
...
create table tenants (
  ...
  created_at timestamptz default now(),
  name       text not null,
  -- GDPR NOTE: EGN is Bulgarian national ID (PII). Stored encrypted at application layer before insertion. Never log or expose raw values.
  egn        text,
  phone      text,
  email      text,
  whatsapp   text,
...
create policy "owner access" on tenants for all using (auth.uid() = owner_id) with check (auth.uid() = owner_id);
...
create table tenancies (
  ...
  payment_day  smallint default 1 check (payment_day between 1 and 31),
  status       text check (status in ('active','expired','terminated'))
);
alter table tenancies enable row level security;
create policy "owner access" on tenancies for all using (auth.uid() = owner_id) with check (auth.uid() = owner_id);
